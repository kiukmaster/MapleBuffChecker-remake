...
export class AlertController {
  ...
  _state(id) {
    ...
    if (!s) {
      s = { active: false, lastPlayed: 0, armed: false, fired: false, timer: null, startTimer: null, interval: null, lastStart: 0 };
      this.state.set(id, s);
    }
    return s;
  }
  ...
  _feedTimer(skill, match, now) {
    ...
    if (s.interval || s.startTimer) return; // already counting or waiting to start
    if (now - s.lastStart < (skill.alert.ignoreMs || 0)) return; // re-cast ignore window

    s.lastStart = now;
    const total = skill.alert.countdownSec || 30;
    const lead = this.leadSec.get(skill.id) ?? skill.alert.defaultLeadSec ?? 5;
    const startDelayMs = skill.alert.startDelayMs || 0;

    this.setDetected(skill.id, true);
    this.setCountdown(skill.id, total); // show full while waiting for the delay

    // Begin the 1-second countdown (optionally after startDelayMs).
    const begin = () => {
      s.startTimer = null;
      let remaining = total;
      let rang = false;
    ...
    if (startDelayMs > 0) {
      s.startTimer = setTimeout(begin, startDelayMs);
    } else {
      begin();
    }
  }
