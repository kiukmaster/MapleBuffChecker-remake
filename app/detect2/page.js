...
export default function Detect2Page() {
  ...
  const [sel, setSel] = useState(null);       // { x, y, w, h } in monitor display px
  const [vAspect, setVAspect] = useState(16 / 9);

  const onDown = (e) => {
    if (!d.isSelecting) return;
    const box = monitorRef.current.getBoundingClientRect();
  ...
  const onMove = (e) => {
    ...
    const box = monitorRef.current.getBoundingClientRect();
    const cx = Math.max(0, Math.min(e.clientX - box.left, box.width));
    const cy = Math.max(0, Math.min(e.clientY - box.top, box.height));
  ...
  const confirmRegion = () => {
    const v = d.videoRef.current;
    const box = monitorRef.current.getBoundingClientRect();
    if (!v || !sel || !box.width) return;
    const sx = v.videoWidth / box.width, sy = v.videoHeight / box.height;
    d.actions.confirmRegion({ x: sel.x * sx, y: sel.y * sy, w: sel.w * sx, h: sel.h * sy });
  };
  const startCapture = () => { setSel(null); d.actions.start(); };
  ...
  return (
    <main className="page detect">
      ...
      <header className="head">
        <div>
          <h1>스킬 감지 · 영역 지정</h1>
          <p className="muted sub">화면 공유 후, 화면에서 <b>감지할 영역만 사각형으로 지정</b>해 그 부분만 감지합니다. 영역이 작을수록 더 가볍게 동작합니다.</p>
        </div>
        <div className={'status' + (d.isDetecting ? ' on' : '')}>
          <span className="dot">{d.isDetecting && <span className="ring" />}</span>
          {d.isDetecting ? '감지 중' : d.isSelecting ? '영역 지정' : d.phase === 'starting' ? '준비 중' : '대기'}
        </div>
      </header>
      ...
      <div className="toolbar">
        ...
        {!d.isActive ? (
          <button className="btn primary" disabled={!anyEnabled} onClick={startCapture}>
            화면 공유 시작
          </button>
        ) : (
      ...
      <div className="layout">
        ...
        <section className="col">
          <h2>{d.isSelecting ? '감지 영역 지정' : '실시간 화면'}</h2>
          <div
            className={'monitor' + (d.isSelecting ? ' selecting' : '')}
            ref={monitorRef}
            style={d.isActive ? { aspectRatio: String(vAspect) } : undefined}
          ...
          >
            ...
            <video
              ...
              muted
              playsInline
              onLoadedMetadata={(e) => { const t = e.target; if (t.videoWidth) setVAspect(t.videoWidth / t.videoHeight); }}
              style={{ display: d.isActive ? 'block' : 'none' }}
            />
            {(d.isSelecting || d.isDetecting) && sel && sel.w >= 4 && (
              <div className="sel-rect" style={{ left: sel.x, top: sel.y, width: sel.w, height: sel.h }} />
            )}
            {d.isSelecting && (!sel || sel.w < 8) && <div className="sel-hint">감지할 영역을 드래그해서 지정하세요</div>}
            {!d.isActive && <span className="no-signal">화면 미연결</span>}
          </div>

          {d.isSelecting && (
            <div className="region-actions">
              <span className="muted small">{sel && sel.w >= 8 ? `선택 영역 ${Math.round(sel.w)} × ${Math.round(sel.h)}` : '영역을 드래그하세요'}</span>
              <div className="spacer" />
              <button className="btn ghost" onClick={d.actions.stop}>취소</button>
              <button className="btn primary" disabled={!sel || sel.w < 8 || sel.h < 8} onClick={confirmRegion}>이 영역으로 감지</button>
            </div>
          )}
      ...
      <style jsx>{`
        ...
        .monitor.selecting { cursor: crosshair; }
        .sel-rect {
          position: absolute; border: 2px solid var(--accent);
          background: rgba(124,196,102,0.10);
          box-shadow: 0 0 0 9999px rgba(0,0,0,0.5);
          pointer-events: none; z-index: 3;
        }
        .sel-hint {
          position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
          color: var(--accent-hover); font-size: 14px; font-weight: 600; pointer-events: none; z-index: 2;
          text-shadow: 0 1px 3px rgba(0,0,0,0.6);
        }
        .region-actions { display: flex; align-items: center; gap: 10px; margin-top: 4px; }
        .region-actions .spacer { flex: 1; }
        .region-actions .small { font-size: 13px; }
